...
class conf_functions:
    ...
    def RMSD_filter(self, hydrogens, tolerance):
        rmsd_matrix = {}
        for i, conf_i in enumerate(self.confs):
            rmsd_matrix[conf_i] = {}
            xyz_i = [value for key, value in self.confs[conf_i]['xyz'].items() if not (hydrogens == False and key.startswith('H'))]
            for j, conf_j in enumerate(self.confs):
                if i == j:
                    rmsd_matrix[conf_i][conf_j] = 0.0
                else:
                    xyz_j = [value for key, value in self.confs[conf_j]['xyz'].items() if not (hydrogens == False and key.startswith('H'))]
                    rmsd_value = rmsd.kabsch_rmsd(xyz_i, xyz_j)

                    rmsd_matrix[conf_i][conf_j] = rmsd_value

        to_remove = set()

        for i, conf1 in enumerate(rmsd_matrix.keys()):
            if conf1 in to_remove:
                continue
            for conf2 in list(rmsd_matrix.keys())[i+1:]:
                if conf2 in to_remove:
                    continue
                if rmsd_matrix[conf1][conf2] < tolerance:
                    if self.confs[conf1] > self.confs[conf2]:
                        to_remove.add(conf1)
                        break
                    else:
                        to_remove.add(conf2)

        return rmsd_matrix, to_remove

    def boltzmann_wights(self):
        print('non')
